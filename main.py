            ...
            try:
                ...
                for space in json.loads(ads_data):
                    for ad in space.get('adverts', []):
                        ...
                        self.session.post(API_URLS['ad_submit'], data=payload, headers={'Secret': self._get_secret(s_key)})
...
if __name__ == '__main__':
    ...
    total_target_per_acc = 20
    batch_per_acc = 20

    print(f"=== 多账户轮询启动 | 账户总数: {len(workers)} ===")
